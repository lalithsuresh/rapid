...
public class WatermarkBufferTest {
    ...
    public void waterMarkTest() {
        ...
        for (int i = 0; i < H - 1; i++) {
            ret = wb.aggregateForProposal(new LinkUpdateMessage(
                    HostAndPort.fromParts("127.0.0.1", i + 1), dst, LinkStatus.UP, configurationId));
            assertEquals(0, ret.size());
            assertEquals(0, wb.getNumProposals());
        }

        ret = wb.aggregateForProposal(new LinkUpdateMessage(
                HostAndPort.fromParts("127.0.0.1", H), dst, LinkStatus.UP, configurationId));
    ...
    public void waterMarkTestBlockingOneBlocker() {
        ...
        for (int i = 0; i < H - 1; i++) {
            ret = wb.aggregateForProposal(new LinkUpdateMessage(
                    HostAndPort.fromParts("127.0.0.1", i + 1), dst1, LinkStatus.UP, configurationId));
            assertEquals(0, ret.size());
            assertEquals(0, wb.getNumProposals());
        }
        ...
        for (int i = 0; i < H - 1; i++) {
            ret = wb.aggregateForProposal(new LinkUpdateMessage(
                    HostAndPort.fromParts("127.0.0.1", i + 1), dst2, LinkStatus.UP, configurationId));
            assertEquals(0, ret.size());
            assertEquals(0, wb.getNumProposals());
        }

        ret = wb.aggregateForProposal(new LinkUpdateMessage(
                HostAndPort.fromParts("127.0.0.1", H), dst1, LinkStatus.UP, configurationId));
        assertEquals(0, ret.size());
        assertEquals(0, wb.getNumProposals());

        ret = wb.aggregateForProposal(new LinkUpdateMessage(
                HostAndPort.fromParts("127.0.0.1", H), dst2, LinkStatus.UP, configurationId));
    ...
    public void waterMarkTestBlockingThreeBlockers() {
        ...
        for (int i = 0; i < H - 1; i++) {
            ret = wb.aggregateForProposal(new LinkUpdateMessage(
                    HostAndPort.fromParts("127.0.0.1", i + 1), dst3, LinkStatus.UP, configurationId));
            assertEquals(0, ret.size());
            assertEquals(0, wb.getNumProposals());
        }
        ...
        ret = wb.aggregateForProposal(new LinkUpdateMessage(
                HostAndPort.fromParts("127.0.0.1", H), dst3, LinkStatus.UP, configurationId));
        assertEquals(0, ret.size());
        assertEquals(0, wb.getNumProposals());
    ...
    public void waterMarkTestBlockingMultipleBlockersPastH() {
        ...
        wb.aggregateForProposal(new LinkUpdateMessage(
                HostAndPort.fromParts("127.0.0.1", H), dst1, LinkStatus.UP, configurationId));
        ret = wb.aggregateForProposal(new LinkUpdateMessage(
                HostAndPort.fromParts("127.0.0.1", H + 1), dst1, LinkStatus.UP, configurationId));
        assertEquals(0, ret.size());
        assertEquals(0, wb.getNumProposals());
        ...
        wb.aggregateForProposal(new LinkUpdateMessage(
                HostAndPort.fromParts("127.0.0.1", H), dst3, LinkStatus.UP, configurationId));
        ret = wb.aggregateForProposal(new LinkUpdateMessage(
                HostAndPort.fromParts("127.0.0.1", H + 1), dst3, LinkStatus.UP, configurationId));
        assertEquals(0, ret.size());
        assertEquals(0, wb.getNumProposals());
